/**
 * @Time  : 2020-01-07 10:10:03
 * @Author: michael huang
 * @File  : axios.js
 * @LastEditors:  michael huang
 * @LastEditTime: 2020-01-07 10:10:03
 */
...
function checkHttpData(response) {
  if (response.status === 200 || response.status === 304 || response.status === 400) {
    if (response.data.errorCode === -999) {
      ...
      let optionJson = response.config.headers['cyj-option'];
...
class HttpRequest {
  post(url, data = {}, option) {
    ...
    return new Promise((resolve, reject) => {
      ...
      toastLoading.show();
      axios.post(api+url, data, { headers: headers, transformRequest: transformRequest }).then(response => {
          //------------------------------------------------------------------------------------------
          // removePending(response.config); //在一个ajax响应后再执行一下取消操作，把已经完成的请求从pending中移除
          //-------------------------------------------------------------------------------------------
          if (response.data.errorCode === 0 || (option && option.returnAll)) {
