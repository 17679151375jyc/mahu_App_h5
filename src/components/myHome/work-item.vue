  ...
  <div style="width: 100vw;background-color: white" @click="itemWorkOnClick">
    ...
    >
      <div class="base-hor-layout ">
        ...
        <p
          class="base-text"
          style="margin-right: 2.6vw;font-size: 3.8vw"
        >
          {{ timeStr }}
        </p>
        <little-gray-dot-icon
          v-if="myWordInfo.messageType !== '安装'"
          :icon="require('@/components/myHome/message/icon_alarm-msg-msg.png')"
          :num="msgNumStr"
          :isForbidden="myWordInfo.isRead"
          @onDotClick="
            showMsgCollectList(
              myWordInfo.messageType,
              myWordInfo.number,
              myWordInfo.plotName +
                myWordInfo.buildingName +
                myWordInfo.doorName
            )
          "
        >
        ...
        <little-gray-dot-icon
          v-if="myWordInfo.messageType !== '安装' && myWordInfo.photoCount > 0"
      ...
      <p class="base-text-details-normal-red" :style="stateStrStyle">
        {{ stateStr }}
      </p>
    </div>
    <div
      class="base-horizontal-layout-space-between-item-center last-line"

    >
      <div>
        <div
          v-if="myWordInfo.messageType === '报警'"
          class="base-column" style=";width: 45vw">
          <p class="my-home-work-item-text" style="margin-bottom: 1.6vw;line-height: 4vw">
            {{ myWordInfo.plotName }}{{ myWordInfo.buildingName }}{{ myWordInfo.doorName }}-{{
              myWordInfo.defenceAreaName === null ||
              myWordInfo.defenceAreaName === undefined ||
              myWordInfo.defenceAreaName === ""
                ? "全屋"
                : myWordInfo.defenceAreaName }}
          </p>
          <!--<p class="my-home-work-item-text" style="margin-bottom: 1.6vw">{{myWordInfo.location}}-{{myWordInfo.defenceArea}}</p>-->
          <p class="my-home-work-item-text">
            报警级别：<span style="color: red">{{ myWordInfo.type }}</span>
          </p>
        </div>
        <div
          v-else-if="myWordInfo.messageType === '维保'"
          class="base-column"
          style="justify-content: space-between;width: 45vw">
          <p class="my-home-work-item-text" style="margin-bottom: 1.6vw">
            {{ myWordInfo.plotName }}{{ myWordInfo.buildingName
            }}{{ myWordInfo.doorName }}-{{
              myWordInfo.defenceAreaName === null ||
              myWordInfo.defenceAreaName === undefined ||
              myWordInfo.defenceAreaName === ""
          ...
          <p class="my-home-work-item-text">维保：{{ myWordInfo.name }}</p>
        </div>
        <div
          v-else-if="myWordInfo.messageType === 'install'"
          class="base-column"
      ...
      <div
        v-show="
          myWordInfo.status !== '误报' &&
            myWordInfo.status !== '已忽略' &&
            myWordInfo.status !== '评价完成'
        "
        class="base-hor-layout two-btn "
        style=""
      >
        <div
          v-if="beingMsgText === ''"
          class="base-hor-layout "
        >
          <button-border
            v-show="button1Text !== ''"
          ...
          <button-border
            style="margin-left: 2.3333vw"
            :buttonBorderStyle="
              myWordInfo.status === '未处理'
        ...
        <div v-else>
          ...
          <p
            class="my-home-work-item-msg-text"
            style="text-align: right"
            v-html="beingMsgText"
          ></p>
        </div>
...
export default {
  ...
  watch: {
    ...
    remainingTime() {
      ...
      if (self.remainingTime <= 0) {
        self.flowTypeStrSome(
          "请求出警",
          {},
          self.messageInfo.requestTime,
          "请求出警",
          "",
          "",
          "即将出警"
        ...
        self.flowTypeStrSome(
          ...
          "",
          "",
          self.messageInfo.remainingTime > 0
            ? "等待安保接警<br>" +
                utils.secondTrans(self.remainingTime) +
                "后将由系统分配出警"
            : "即将出警"
        );
      }
  ...
  methods: {
    getRemainingTimeInfo() {
      let self = this;
      if (
        self.myWordInfo.messageType === "报警" &&
        self.myWordInfo.status === "请求出警"
      ) {
        ...
        self.flowTypeStrSome(
          "请求出警",
          {},
          self.myWordInfo.requestTime,
          "",
          "",
          self.myWordInfo.remainingTime > 0
            ? "等待安保接警<br>" +
                utils.secondTrans(self.remainingTime) +
                "后将由系统分配出警"
            : "即将出警"
    ...
    getMyWordItemInfo() {
      ...
      if (this.myWordInfo.messageType === "报警") {
        ...
        switch (this.myWordInfo.status) {
          case "未处理":
            this.flowTypeStrSome(
              "未处理",
              {},
              this.myWordInfo.createTime,
              "立即出警",
              "误报",
              ""
            );
        ...
        this.messageTypeStrSome(
          require("@/assets/icon/icon_install.png"),
          "",
          ""
        );
    ...
    handleImgsClick() {
      ...
      let self = this;
      self
        .$post("alertMessage", "/getAlertPhoto", {
          alertNumber: self.myWordInfo.number
        })
        .then(res => {
          let imgs = [];
    ...
    alertOnClick(num) {
      ...
      if (num === 1) {
        if (this.myWordInfo.messageType === "报警") {
          switch (this.myWordInfo.status) {
            case "未处理":
              self.$emit("alertOnClick", {
                number: self.myWordInfo.number,
                name: self.myWordInfo.name
              });
              break;
            case "已接警":
              self.$emit("callAlertPersonnelOnClick", {
                phone: self.myWordInfo.securePhone,
                messageType: self.myWordInfo.messageType
              });
              break;
            case "正在出警":
              self.$emit("callAlertPersonnelOnClick", {
          ...
          switch (this.myWordInfo.status) {
            case "未处理":
              self.$emit("maintainOnClick", {
                number: self.myWordInfo.number,
                name: self.myWordInfo.name,
                content: self.myWordInfo.content,
                orderTime: 0
              });
              break;
            case "已接单":
          ...
          switch (this.myWordInfo.status) {
            ...
            case "已接单":
              self.$emit("maintainOnClick", {
                ...
                name: self.myWordInfo.name,
                content: self.myWordInfo.content,
                orderTime: self.myWordInfo.orderTime
              });
              break;
            case "正在处理":
              self.$emit("maintainOnClick", {
    ...
    itemWorkOnClick() {
      ...
      if (this.myWordInfo.messageType === "报警") {
        if (
          this.myWordInfo.status === "未处理" ||
          this.myWordInfo.status === "误报"
        ) {
          return;
        } else {
          this.$router.push({
            path: this.toPath,
            query: { messageType: "报警" }
          });
        }
      } else if (this.myWordInfo.messageType === "维保") {
        if (
          this.myWordInfo.status === "未处理" ||
          this.myWordInfo.status === "已忽略"
        ) {
          return;
        } else {
          this.$router.push({
            path: this.toPath,
            query: { messageType: "维保" }
          });
        }
        ...
        switch (this.myWordInfo.flowType) {
          ...
          case "being":
            this.$router.push({
              path: this.toPath,
              query: { flowType: "being", messageType: "install" }
            });
            break;
          case "receive":
            this.$router.push({
              path: this.toPath,
              query: { flowType: "receive", messageType: "install" }
            });
            break;
          case "complete":
            this.$router.push({
              path: this.toPath,
              query: { flowType: "complete", messageType: "install" }
            });
            break;
          case "evaluate":
            this.$router.push({
              path: this.toPath,
              query: { flowType: "evaluate", messageType: "install" }
            });
            break;
          case "fault":
            break;
