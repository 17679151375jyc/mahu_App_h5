  ...
  <div style="height:100%">
    ...
    <div v-if="hasLocalData" style="width: 100%;height: 100%">
      ...
      <div style="width: 100%;height: 0;position: relative;top: 0;z-index: 11">
        <!--!locationData.staff.staffType || locationData.staff.staffType!=='alert'-->
        <div v-if="isShowHeader" class="header">
          <p id="headStatus">
            正在获取位置信息，请打开定位 <span class='fa fa-circle-o-notch fa-spin' style='color:rgba(51,136,255,0.8)'></span>
          </p>
          <!--<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i>-->
          <img v-if="isShowLoad" :src="require('@/assets/icon/icon-loading.gif')"
          ...
          utils.localStorage('locationData', this.locationData).then((res) => {
            ...
            map.plugin('AMap.Geolocation', function () {
              ...
              function onComplete(data) {
                ...
                console.log("定位lng",data.position.lng);
                console.log("定位lat",data.position.lat);
                ...
                AMapUI.loadUI(['overlay/SimpleMarker'], function (SimpleMarker) {
                  ...
                  //标记技术人员位置,创建点
                  for (let i = 0,l=customize.staff.staffLocation?customize.staff.staffLocation.length:0; i < l; i++) {
                    // 创建一个 Icon
                    ...
                    if (customize.staff.staffType === "alert") {
                      if (i===0) {
                        let srcImgHome = require('./icon_home_white.png');
                        markerContent = '' +
                        ...
                        markerContent = '' +
                          '<div class="amap-custom-content-marker" style="background: rgba(51, 136, 255, 1)">' +
                          '   <img src="'+srcImg+'">' +
                          '</div>';
                      } else {
                        let srcImgHome = require('./icon_home_white.png');
                    ...
                    function geocoderAlert_CallBack(index, data, isTop) {
                      ...
                      let v =
                        ...
                        // + data.regeocode.addressComponent.street + data.regeocode.addressComponent.streetNumber;
                      let re = new RegExp(v + "(\\S*)","");
                      let addressStr = data.regeocode.formattedAddress.match(re)[1];
                      ...
                      marker.setLabel({
                        ...
                        content:  "<div class='amap-custom-content-marker-info'>" + title + "位置 " + addressStr + "</div>"
                        // 江门市环市二路10号幸福豪庭附近
                      });
                      map.add(marker);
