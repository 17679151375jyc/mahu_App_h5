...
<template>
  <div class="visitor-item-index-box"
       :class="{'base-vertical-layout-space-between':true,
             'base-background-img-blue-light':true}">
    <div class="base-horizontal-layout-space-between-item-center" style="margin: 4vw 5.3333vw 0 5.3333vw">
      <div class="base-horizontal-layout-center-item-center">
        <p class="visitor-item-index-title">{{visitorIndexInfo.openid?visitorIndexInfo.visitorName:visitorIndexInfo.roleName}}</p>
        <img v-if="!visitorIndexInfo.recordType===1"
        ...
        <p v-if="!visitorIndexInfo.recordType===1"
           class="base-text-details-large-black-666"
           style="color: white">{{visitorIndexInfo.vistorPhone}}</p>
      </div>
      ...
      <div v-if="!visitorIndexInfo.openid" :class="['base-vertical-layout-center-item-center']"
           class="visitor-item-index-btn-border" @click="btnOnClick(0,visitorIndexInfo.tempVisitorNumber,visitorIndexInfo.triggeringTime)">
    ...
    <div class="base-horizontal-layout-space-between" style="padding: 5.3333vw">
      ...
      <div v-if="visitorIndexInfo.openid" class="base-vertical-layout-space-between"
           :class="{'base-text-details-large-white':true}" style="height: 18vw">
        <p>{{visitorIndexInfo.plotName}}{{visitorIndexInfo.buildingName}}</p>
        <p class="base-text-details-normal-white"
           style="color: rgba(255,255,255,0.7);"
        >微信访客</p>
        <p>{{getTime(visitorIndexInfo.startTime)}}</p>
      </div>
      ...
      <div v-else class="base-vertical-layout-space-between">
        <div class="visitor-item-index-btn-color"
          ...
          <img :src="require('@/assets/icon/icon-cut-pic.png')" class="base-icon-normal-style" style="margin-right: 1.0666vw">
          <p class="base-text-details-normal-white">查看摄图</p>
        </div>
        <div class="visitor-item-index-btn-color"
             :class="['base-horizontal-layout-center-item-center']"
             style="margin-top: 2.6666vw"
          ...
          >{{visitorIndexInfo.openStatus===0?'开门':
            ...
            visitorIndexInfo.openStatus===2?'开门中':''}}</p>
        </div>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      btnOnClick(index,recordNumber,recordTime) {
        switch (index) {
          case 0:
            this.$emit('ignoreOnClick', {openStatus: this.visitorIndexInfo.openStatus, recordNumber: recordNumber, recordTime: recordTime});
            break;
          case 1:
            this.lookPic();
            break;
          case 2:
            this.$emit('openDoorOnClick', {openStatus: this.visitorIndexInfo.openStatus, recordNumber: recordNumber, recordTime: recordTime});
            break;
          default:
            break;
        }
