<template>
  <div class="verification-button-box">
    <cube-button class="verification-button-text" :primary="true" :disabled="mCodeBtnDisabled" @click="sendVerification">
      <div>
  ...
  export default {
    ...
    props:
      {
        ...
        codeBtnDisabled: {//todo 按钮两种情况的不可点击，后续再优化
          type: Boolean,
          default: false,
        }
      },
    data() {
      return {
        codeBtnText: "发送验证码",
        countDown: 60,
        codeBtnStyle: {color: '#fff',fontSize: '3.7333vw'},
    ...
    created() {
      ...
      if(!this.phoneNumber){
        this.mPhoneNumber =  JSON.parse(localStorage.getItem('userInfo')).infos.userPhone;
      }
    ...
    watch: {
      mCodeBtnDisabled() {
        if(this.mCodeBtnDisabled) {
          this.codeBtnStyle = {color: '#fff',fontSize: '3.2vw'};
        } else {
          this.codeBtnStyle = {color: '#fff',fontSize: '3.7333vw'};
        }
      },
      countDown() {
        if(this.countDown===0) {
