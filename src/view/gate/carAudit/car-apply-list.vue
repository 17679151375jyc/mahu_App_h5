...
<template>
  <div class="wrapper-white">
    ...
    <cube-tab-bar @click='tabClick' :showSlider='false' v-model="selectedLabel" show-slider style="width: 100%;height: 13.33vw;background-color: #F5F5F5;">
      <cube-tab v-for="(item, index) in tabs" :label="item.label" :key="index" class='header'>
      </cube-tab>
    </cube-tab-bar>
    <cube-scroll style="width: 100%;height: 88%;">
      <cube-tab-panels v-model="selectedLabel" style="width: 100%;height: 80%;">
        <cube-tab-panel v-for="(item, index) in tabs" :label="item.label" :key="index" style="width: 100%;height: 85%;">
          <ul class="ulClass ul_bjall">
            <li v-if='list.length==0' class="null_class">暂无业主申请加入社区</li>
            <li @click='detail(index)' :class="{'ul_li bg_no':item.status == 0||item.status == 3,'ul_li bg_ok':item.status == 1, 'ul_li bg_pass':item.status == 2}" v-for='(item, index) in list' :key='index'>
              <div style='height: 6.67vw;line-height: 6.67vw;left:5.33vw;margin-top:3.2vw;right: 5.33vw;'>
                <span style='font-size: 4.8vw;font-weight: bold;margin-right: 1.87vw;'>{{ item.carNumber }}</span>
                ...
                <span style='display: block;float: right;font-size: 3.73vw;'>{{ statusList[item.status].name }}</span>
              </div>
              <div style='height: 5.33vw;margin-top: 13.33vw;left:5.33vw;right: 5.33vw;line-height: 5.33vw;font-size: 3.73vw;'><span  style='margin-right: 3.2vw'>{{item.carownName}} {{item.carownPhone}}</span></div>
              <div style='height: 5.33vw;margin-top: 18.33vw;left:5.33vw;right: 5.33vw;line-height: 5.33vw;font-size: 3.73vw;'><span  style='margin-right: 3.2vw'>{{item.plotName}}</span></div>
              <div style='height: 5.33vw;margin-top: 21.33vw;left:5.33vw;right: 5.33vw;line-height: 5.33vw;'>{{ null }}</div>
              <div style='height: 5.33vw;margin-top: 29.07vw;left:5.33vw;right: 5.33vw;line-height: 5.33vw;font-size: 3.73vw;'
              >{{ getDateTime(item.updateTime) }}</div>
              <div v-if='item.status == 2' @click='detail(index)' style='width: 26.13vw;height: 8.53vw;line-height: 8.53vw;text-align: center;right: 4vw;bottom: 3.74vw;background: linear-gradient(to top , #F9BC30, #FD484D);border-radius:1.07vw;font-size: 3.2vw;'>拒绝原因</div>
            </li>
          </ul>
  ...
  export default {
    ...
    methods: {
      ...
      detail(val) {
        ...
        this.$router.push({path: "/service-index/car-apply-list/car-apply-audit", query: {}});
        // }
      },
      tabClick(val) {
        if (val === '全部') {
